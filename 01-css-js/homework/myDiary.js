...
window.onload = () => {
    if (JSON.parse(sessionStorage.getItem("userID"))) {
        ...
        document.querySelector(
            ".footer__copy"
        ).innerText = `Copyright © 2024. ${userID} `;
    } else {
...
window.addEventListener("scroll", () => {
    ...
    if (window.scrollY < 360) {
        document.querySelector(".inner__text").style =
            "background-color: #fff; color: #222; transition: 0.2s;";
        document.querySelector(".container__sticky").style = "display: none;";
    } else if (footerHeight <= currentHeight) {
        document.querySelector(".container__floating").style =
            "position: absolute; bottom: 3%; right: 5%;";
    } else {
        document.querySelector(".inner__text").style =
            "background-color: #222; color: #fff; transition: 0.2s;";
        document.querySelector(".container__sticky").style = "display: block;";
        document.querySelector(".container__floating").style =
            "position: fixed; bottom: 2%; right: 4%;";
    }
});
...
function openFilter() {
    ...
    document
        ...
        .addEventListener("click", (event) => {
            ...
            document.querySelector(
                ".inner__text"
            ).style.cssText = `--innerTextContent: "${
                moodIndex[event.target.id]
            }"`;
            ...
            switch (event.target.id) {
                ...
                case "happy": {
                    filterDiary = diaryLocal.filter(
                        (el) => el.mood === "happy"
                    );
                    sessionStorage.setItem(
                        "tempData",
                        JSON.stringify(filterDiary)
                    );
                    makeDiaryCard(
...
function searchDiary() {
    ...
    const searchResult = diaryLocal.filter((el) =>
...
function loadIMG() {
    const getIMG = () => {
        setTimeout(() => {
            document
                .querySelectorAll(".gallery__imgBox img")
                .forEach((el) => (el.style = "opacity: 100%;"));
            document
                .querySelectorAll(".img__skeleton")
    ...
    window.addEventListener("scroll", () => {
        ...
        const scrollPer =
            callDocument.scrollTop /
            (callDocument.scrollHeight - callDocument.clientHeight);
        ...
        case "가로형": {
            document
                ...
                .forEach(
                    (el) =>
                        (el.style =
                            "width: 48rem; height: auto; aspect-ratio: 4 / 3; opacity: 100%;")
                );
            break;
        }
        case "세로형": {
            document
                ...
                .forEach(
                    (el) =>
                        (el.style =
                            "width: auto; height: 48rem; aspect-ratio: 3 / 4; opacity: 100%;")
                );
            break;
        }
        default: {
            document
                ...
                .forEach(
                    (el) =>
                        (el.style =
                            "width: 48rem; height: auto; aspect-ratio: 1 / 1; opacity: 100%;")
                );
            break;
        }
    }
...
function changeBG(event) {
    ...
    debounce = setTimeout(() => {
        ...
        document.querySelector(
            ".overlay"
        ).style.background = `linear-gradient( ${mouseX}deg,
        #efeeee,
        #faf1ea,
        #f2f8ea,
        #e7f9ef,
...
function closeDiary() {
    ...
    setTimeout(() => {
        diaryForm.style =
            "transform: translateX(500px) translateY(-50px) rotate(10deg); opacity: 0; transition: 1s; transition-timing-function: ease-out";
    }, 100);
    setTimeout(() => {
        diaryForm.style.display = "none";
        document.querySelector(".container__diaryForm").style =
            "display: flex;";
    }, 1000);
}
...
//** 새로운 모달 호출 */
function newDiary() {
    document.querySelector(".container__modal").style =
        "width: 100%; height: 100%;";
    document.querySelector("body").classList.add("stop-scrolling");
    document.querySelector(".modal__bg").style = "display: flex;";
    document.querySelector(".modal__form").style =
        "display: flex; filter: blur(0px) brightness(100%);";
...
function exitNew() {
    document.querySelector(".modal__form").style =
        "display: flex; filter: blur(2px) brightness(50%);";
...
function closeNew() {
    ...
    document.querySelector(".modal__form").style =
        "display: none; filter: blur(0px) brightness(100%);";
...
let itemsPerPage = 8;
const lastPage =
    diaryLocal !== null ? Math.ceil(diaryLocal.length / itemsPerPage) : "1";
//** 페이지네이션 버튼 만들기 */
function pagination() {
    const dummyData = new Array(10).fill("dummy");
    const makePageNum = dummyData
        .map((el, idx) => {
        ...
        <button
            ...
            pagination()";
            class="${
                clickedPage === pageNum ? "page__select" : "page__button"
            }";
        > ${pageNum}</button>`
                : "";
        })
...
function pageList(page) {
    if (diaryLocal !== null) {
        ...
        const diaryPerPage = diaryLocal?.filter((_, idx) => {
            const skipThis = (page - 1) * itemsPerPage;
            if (skipThis <= idx && idx < skipThis + itemsPerPage) {
                return true;
            } else {
                return false;
            }
        });
        document.querySelector(".body__left__card").innerHTML = diaryPerPage
            .map(
                (el, idx) =>
                    `<div class="wrapper__card">
...
document
    .querySelector(".gallery__direction")
    .addEventListener("change", changeDirection);
...
document
    .querySelector(".diary__button")
    .addEventListener("click", makeDiaryData);

document
    .querySelector(".page__prev")
    .addEventListener("click", () => pageTest("prev"));
document
    .querySelector(".page__next")
    .addEventListener("click", () => pageTest("next"));
