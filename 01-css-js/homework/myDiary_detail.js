...
function moveToComment() {
    const commentSection = document
        .querySelector(".comment__list")
...
`;
        ...
        document
            ...
            .addEventListener("click", editDiary);
...
function deleteDiary() {
    localDiary.splice(indexNum, 1);

    localStorage.setItem("diaryData", JSON.stringify(localDiary));
...
function submitComment() {
    ...
    currentDiary.comment.push(diaryComment);

    localStorage.setItem("diaryData", JSON.stringify(currentDiary));
    document.querySelector(".input__text").value = "";

    makeDiaryComment(currentDiary);
}
...
function makeDiaryComment() {
    if (currentDiary.comment[0] !== undefined) {
        ...
        commentList.innerHTML = currentDiaryComment
            .map(
                (el) =>
                    `
