...
const validateInputText = () => {
    ...
    moodChecked = checkedInputMood()
    activateWriteButton(moodChecked)
}

const checkedInputMood = () => {
    const moodList = document.getElementsByName("mood_radio")
    for (let i = 0; i < moodList.length; i = i + 1) {
        if (moodList[i].checked) {
            selectedMood = moodList[i].value
...
const activateWriteButton = (moodChecked) => {
    if (inputTitle !== "" && inputText !== "" && moodChecked === true) {
    ...
    if (inputTitle === "" || inputText === "" || moodChecked === false) {
