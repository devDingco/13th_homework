...
import { CREATE_BOARD, UPDATE_BOARD } from '@/commons/graphql/backend-api'
...
const BoardsWrite = (props: any) => {
  ...
  const errorMessage = props.isEdit ? "" : "필수 입력 사항입니다."
  ...
  function onChangeTitle(event: ChangeEvent<HTMLInputElement>) {
    ...
    return props.isEdit ? setIsActive(true) : setIsActive(false)
  }
  ...
  async function onClickEditButton() {
    const passwordInput = prompt("글을 입력할 때, 입력하셨던 비밀번호를 입력해주세요.")
  ...
  const RegisterForm = () => {
    ...
    const buttonAction = props.isEdit ? onClickEditButton : onClickSubmitButton

    return (
        <>
            <button className={styles.cancelButton} onClick={onClickCancel}>취소</button>
            <button className={styles.submitButton} style={isActive ? submitButtonStyle : disabledSubmitButtonStyle} onClick={buttonAction}>{pageTitle}하기</button>
        </>
    );
  }
      ...
      <div className={styles.BoardsNew_rootContainer}>
          <header className={styles.BoardsNew_header}>게시물 {pageTitle}</header>
            ...
            <div className={styles.BoardsNew_UserInputForm}>
                  <BasicInputForm isRequired={true} title="작성자" placeholder={writerDescription} onChangeHandler={onChangeWriter} errorMessage={writerErrorMessage} isEdit={props.isEdit} defaultValue={board.writer}/>
                  <BasicInputForm isRequired={true} title="비밀번호" placeholder={passwordDescription} onChangeHandler={onChangePassword} errorMessage={passwordErrorMessage} isEdit={props.isEdit} defaultValue={props.isEdit ? "********" : ""}/>
              </div>
              <Divider />
              <BasicInputForm isRequired={true} title="제목" placeholder={titleDescription} onChangeHandler={onChangeTitle} errorMessage={titleErrorMessage} defaultValue={board.title}/>
              <Divider />
              <ContentsInputForm title="내용" placeholder={contentsDescription} onChangeHandler={onChangeContents} errorMessage={contentsErrorMessage} defaultValue={board.contents}/>
              <AddressInputForm />
              <Divider />
  ...
  if (props.isRequired) {
      return (
          <div id={styles.PostInputForm} className={styles.BoardsNew_inputForm}>
              ...
              <TextInput isEdit={props.isEdit} placeholder={props.placeholder} onChangeHandler={props.onChangeHandler} defaultValue={props.defaultValue}/>
...
const ContentsInputForm = (props: any) => {
  return (
      <div className={styles.BoardsNew_inputForm}>
          <div className={styles.inputTitle}>{props.title}<span className={styles.requiredMark}>*</span></div>
          <textarea className={styles.inputTextArea} placeholder={props.placeholder} onChange={props.onChangeHandler} defaultValue={props.defaultValue}></textarea>
          <p className={styles.inputError}>{props.errorMessage}</p>
      </div>
  );
}
...
const TextInput = (props: any) => {
  return <input className={styles.inputText} type="text" placeholder={props.placeholder} onChange={props.onChangeHandler} defaultValue={props.defaultValue} disabled={props.isEdit}/>;
}
