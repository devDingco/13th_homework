...
export default function CommentUI({ el, current, setCurrent }) {
    ...
    return (
        <>
            ...
            <pre className={css({ p: "1rem 0rem" })}>
                {current === el._id ? (
                    <Input id="comment_ID" value={el.contents} onChange={handleChange} />
                ) : (
                    el.contents
                )}
            </pre>
            ...
            <div className={css({ fontSize: "1.4rem" })}>
                {current === el._id ? <Input id="password_ID" onChange={handleChange} /> : el.createdAt.split("T")[0]}
            </div>
        </>
    );
}
