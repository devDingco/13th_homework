...
export default function BoardsDetailUI({ isEdit }: { isEdit: boolean }) {
    ...
    if (data?.fetchBoard.youtubeUrl?.includes("=") || data?.fetchBoard.youtubeUrl?.includes("-")) {
        const link = data?.fetchBoard.youtubeUrl.split("=")[1] || data?.fetchBoard.youtubeUrl?.split("-")[1];
        console.log(link);
    }
                ...
                {isEdit ? (
                    ...
                    <div className={styles.info_link}>
                        ...
                        <Tooltip.Root closeDelay={200} openDelay={100}>
                            ...
                            <Tooltip.Positioner>
                                <Tooltip.Content className={css_tooltip}>
                        ...
                        data?.fetchBoard.images?.map((el) =>
                            el !== "" ? (
                                <Image
                                    ...
                                    width={400}
                                    height={0}
                                    className={css({ w: "40rem", objectFit: "cover", m: "1rem 0rem" })}
                                />
                            ) : (
                                ""
                            ),
