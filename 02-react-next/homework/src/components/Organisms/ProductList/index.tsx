...
import Button from "@/components/Atoms/_Button/index";
...
// import ProductDetailUI from "../Product_Detail/product_detail";

import { useQuery } from "@apollo/client";
import { FETCH_PRODUCT_LIST } from "@/common/queries/queries";

export default function ProductListUI() {
    const { data: products } = useQuery(FETCH_PRODUCT_LIST);
    ...
    return (
        <section>
            ...
            <div className={CSS_Between}>{/* <SearchUI /> */}</div>

            <div className={CSS_List}>
                {products?.fetchTravelproducts.map((el) => <ProductCardUI key={el._id} el={el} />)}
            </div>
        </section>
    );
}
