window.onload = () => {
  ...
  window.addEventListener("scroll" ,() => {
    ...
    if(스크롤내려간길이 > 0) {
      document.getElementById("HTML_필터").style = "background-color: black; color: white"
    } else {
      document.getElementById("HTML_필터").style = "background-color: white; color: black"
    }
})
  // 1. 시작하면 일기 목록에 그리기
  JS_일기그리기기능();
          ...
          <div class="CSS_일기">
              ...
              <div class="CSS_일기사진">
                ...
                ${
                  ...
                  <button class="CSS_버튼"  onclick="목록에서삭제(event, ${index})">X</button>
              </div>
...
const 목록에서삭제 = (event, index) => {
  ...
  const 일기목록 = JSON.parse(스토리지에저장된일기목록);
  console.log(일기목록)
  window.localStorage.removeItem(스토리지에저장된일기목록.index)
  console.log(index)
  alert(`${index}번이 삭제되었습니다.`)
