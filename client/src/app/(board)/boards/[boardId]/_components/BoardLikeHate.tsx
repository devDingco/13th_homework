...
import { IBoardReactionProps } from '@/models/board.type';
import Image from 'next/image';

export default function BoardLikeHate({ data }: IBoardReactionProps) {
	return (
		<div className="flex items-center justify-center gap-6">
			<div className="flex cursor-pointer flex-col items-center gap-1">
				...
				<div className="prose-r_14_20 text-gray-700">{data.hate}</div>
			</div>
			<div className="flex cursor-pointer flex-col items-center gap-1">
				<Image src="/Images/like.svg" alt="like" width={24} height={24} />
				<div className="prose-r_14_20 text-[#F66A6A]">{data.like}</div>
			</div>
		</div>
	);
}
