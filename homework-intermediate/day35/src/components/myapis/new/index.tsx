...
export default function MyApisNew() {
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!isValidFileType(selectedFile)) {
      setError('PNG, JPEG, 또는 JPG 파일만 업로드 가능합니다.');
      setFile(null); // 상태 초기화
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-8 items-center p-16">
      <Image
        className="max-w-lg object-cover rounded-lg"
        src={dogRobberMeme}
        alt="강아지강도짤"
      />
      <input
      ...
      />
      <button
        className="w-16 rounded border-1 bg-blue-600 text-white"
