...
window.onload = () => {
  ...
  // 스크롤
  document.getElementById("일기추가할공간").addEventListener("scroll", () => {
    const 스크롤내려간길이 = document.getElementById("일기추가할공간").scrollTop;
    if (스크롤내려간길이 > 0) {
      document.getElementById("필터").style = "background-color: black; color: white"
    } else {
      document.getElementById("필터").style = "backgrouond-color: white color: black"
    }
  })
  console.log(일기목록)
  일기DOM만들기();
...
const 일기삭제버튼기능 = (event, 삭제할일기번호) => {
  ...
  로컬스토리_일기_삭제후 = 로컬스토리_일기.filter((el, index) => index !== 삭제할일기번호);
