...
const BoardsDetail = () => {
    ...
    return (
        <div className="layout">
            ...
            <div className="head">
            <div className="contentTitle">살어리 살어리랏다 쳥산(靑山)애 살어리랏다멀위랑 ᄃᆞ래랑 먹고 쳥산(靑山)애 살어리랏다얄리얄리 얄랑셩 얄라리 얄라</div> {/* 제목 */}      
            <div className="createInfo"> {/* 프로필 이미지+작성자이름 / 작성일자 <- jsutify-content:space-between, 가로, 아래에 Margin 주고 border-bottom 해서 선 추가 */}
                <div className="writerName">
                    <img src="/img/icon/profile_img.png" className="profileImg" alt="" />
                    <span id="whoWrite">홍길동</span>
                </div>
                <div className="writeDate">2024.11.11</div>
            </div>
            <div className="shareImg"> {/* 링크이미지 + 위치이미지 <- 오른쪽 정렬, 가로 */}
                <img src="/img/icon/link_icon_24.png" className="shareicon" alt="" />
                <img src="/img/icon/location_24.png" className="shareicon" alt="" />
            </div>
            </div>

            <div className="contentBox"> {/* 이미지 + 내용 칸 <- 넓게..., 세로, 이미지랑 텍스트 다 왼쪽 정렬 */}
            ...
            <div className="previewBox"> {/* 동영상 미리보기 (기능x 이미지만 넣기) <- 내용 작성 칸이랑 w 똑같이!!, background-color: lightgray, 가운데 정렬 */}
                <img src="/img/video.png" className="previewImg" alt="" />
            </div>

            <div className="evaluate"> {/* 싫어요 이미지(숫자랑 div로 묶기(세로)), 좋아요 이미지(숫자랑 div로 묶기(세로)) <- 버튼으로 만들고 이미지 넣기 해야겠당... 가로, 가운데 정렬, gap 줘야 함 */}
                <div className="hate">
            ...
            <div className="toButtons">  {/* 목록으로 버튼, 수정하기 버튼 <- 가로, gap 줘야 함, 둘 다 버튼 안에 이미지 넣어야 함 */}
