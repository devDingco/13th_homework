...
import type { IcolumnSet } from "@/components/mypage/point-list/types";
...
export const usePointList = ({
  ...
  const dataSourceSetting = (idx: number) => {
    ...
    const data = pointTransactionsData?.[idx] as DataType; // ! 해당 아이템 인덱스에 맞는 데이터 가져오기
    if (!columns || !data) return;

    // ! columns 에서 key 값에 따라 데이터 매핑
    const keyArr = columns.map((column) => {
      const key = column.key as keyof DataType;
      if (key.includes(".")) {
        // ! columns 에서 key 값이 "."을 포함하는 경우 (travelproduct.name, user.name) 처리
        const [firstKey, secondKey] = key.split(".");
        const firstKeyFind = data[firstKey as keyof DataType];
