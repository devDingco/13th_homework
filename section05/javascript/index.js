window.onload = () => {
	...
	document.addEventListener("DOMContentLoaded", function () {
		const mainElement = document.getElementById("HTML_메인");
	...
	window.addEventListener("click", (event) => {
		const 모달 = document.getElementById("HTML_모달");
		if (event.target === 모달) {
			JS_모달닫기();
		}
	});
...
// 모달 열기
const JS_모달열기 = () => {
	document.getElementById("HTML_모달").style.display = "block";
	// 스크롤 맨 위로 이동
	document.querySelector(".CSS_모달내용").scrollTop = 0;
	// 배경 스크롤 방지
...
// 모달 닫기
const JS_모달닫기 = () => {
	document.getElementById("HTML_모달").style.display = "none";
	// 배경 스크롤 복원
