"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.BoardPasswordService = void 0;
const common_1 = require("@nestjs/common");
const bcrypt_service_1 = require("../../bcrypt/bcrypt.service");
const board_repository_1 = require("../repository/board.repository");
let BoardPasswordService = class BoardPasswordService {
    constructor(boardRepository, bcryptService) {
        this.boardRepository = boardRepository;
        this.bcryptService = bcryptService;
    }
    async validateBoardData(boardId, password) {
        const findBoard = await this.boardRepository.findBoard(boardId);
        if (!findBoard) {
            throw new common_1.NotFoundException(`boardId ${boardId} is not found`);
        }
        await this.bcryptService.validatePassword(password, findBoard.password);
        return true;
    }
};
exports.BoardPasswordService = BoardPasswordService;
exports.BoardPasswordService = BoardPasswordService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [board_repository_1.BoardRepository,
        bcrypt_service_1.BcryptService])
], BoardPasswordService);
//# sourceMappingURL=board-password.service.js.map